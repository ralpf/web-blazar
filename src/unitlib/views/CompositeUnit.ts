...
export abstract class CompositeUnit extends Unit {
    ...
    public attachClassField(fieldName: string, unitInstance: Unit) {
        ...
        log(`\t\t·${this.constructor.name}.${fieldName} field added`);
    }
    ...
    protected getField<T extends Unit>(fieldName: string): T {
        ...
        Assert.False(!unit, `no filed '${this.constructor.name}.${fieldName}' was found (refactored?) Available: [${Object.keys(this.fields).join(", ")}]`);
        return unit as T;
    }
}